...
function STARTER(word) {
    ...
    if (s == "Start Order" || word == "exit") {
        $('#right').fadeToggle(1000);
        $('#left').fadeToggle(1000);
        $('.midButtons').fadeToggle(1000);
        $("#total").fadeToggle(1000);
        $(word).fadeTo(1000, .3);
        document.getElementsByClassName("starter")[0].childNodes[0].innerHTML = "Ordering";
    }
    if (word == "exit") {
        $(document.getElementsByClassName("starter")[0]).fadeTo(1000, 1);
        document.getElementsByClassName("starter")[0].childNodes[0].innerHTML = "Start Order";
    }
}
...
function doTaxes() {
    ...
    moneyTotal = (total * 1.06).toFixed(2);
}

function finalizer() {
    $("#god").fadeIn(1);
    $("#finalize").fadeIn(250);
    var tags = document.getElementById("money").getElementsByTagName("span");
    tax = (total * .06).toFixed(2);
    subTotal = (total * 1.06).toFixed(2);
    document.getElementById("money").getElementsByTagName("span")[0].innerHTML = "Total: $" + moneyTotal;
}

function closeFinalizer() {
    var tag = document.getElementsByTagName("input");
    for (var i = 0; i < tag.length; i++) { tag[i].value = ""; }
...
function money(n) {
    ...
    var tag = document.getElementById("money").getElementsByTagName("span")[0].innerHTML;
    if (n == 0)
        moneyTotal -= document.getElementById("tab-1").getElementsByTagName("input")[0].value;
    else
        moneyTotal = 0;

    document.getElementById("money").getElementsByTagName("span")[0].innerHTML = "Total: $" + moneyTotal.toFixed(2);
    document.getElementById("tab-1").getElementsByTagName("input")[0].value = "";
    if (moneyTotal <= 0) {
        if (n == 0)
            alert("Thanks for ordering!\nYour Change is $" + -1 * moneyTotal.toFixed(2));
        else
            alert("Thanks for ordering!");
        location.reload();
        /*clearer();
